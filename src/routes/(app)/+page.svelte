...
<div class="space-y-6 max-w-7xl mx-auto">
  ...
  <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-2xl font-bold text-white/90 font-sans">Dashboard</h1>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <GlassCard className="p-5">
      <div class="flex items-center justify-between mb-3">
        <span class="text-xs text-white/40 font-sans uppercase tracking-wider">Equity</span>
        <div class="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.6)] animate-glow-emerald"></div>
      </div>
      <p class="text-2xl font-bold font-mono text-white/90">${equity.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
    </GlassCard>
    
    <GlassCard className="p-5">
      <div class="flex items-center justify-between mb-3">
        <span class="text-xs text-white/40 font-sans uppercase tracking-wider">Today's P&L</span>
      ...
      <p class="text-2xl font-bold font-mono {pnlColor}">{pnlSign}{todayPnl.toFixed(2)}</p>
    </GlassCard>
    
    <GlassCard className="p-5">
      <div class="flex items-center justify-between mb-3">
        <span class="text-xs text-white/40 font-sans uppercase tracking-wider">Unrealized</span>
        <div class="w-2 h-2 rounded-full {unrealizedPnl >= 0 ? 'bg-emerald-400' : 'bg-rose-400'}"></div>
      </div>
      <p class="text-2xl font-bold font-mono {unrealizedPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}">{unrealizedPnl >= 0 ? '+' : ''}{unrealizedPnl.toFixed(2)}</p>
    </GlassCard>
    
    <GlassCard className="p-5">
      <div class="flex items-center justify-between mb-3">
        <span class="text-xs text-white/40 font-sans uppercase tracking-wider">Positions</span>
        <div class="w-2 h-2 rounded-full bg-blue-400 shadow-[0_0_8px_rgba(59,130,246,0.6)]"></div>
      </div>
      <p class="text-2xl font-bold font-mono text-white/90">{positions.length}</p>
    </GlassCard>
  </div>
      ...
      >
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Max Risk/Trade</p>
            <p class="text-xl font-mono font-bold text-white/90">2.0%</p>
          </div>
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Max Positions</p>
            <p class="text-xl font-mono font-bold text-white/90">4</p>
          </div>
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Soft Drawdown</p>
            <p class="text-xl font-mono font-bold text-amber-400">3.0%</p>
          </div>
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Hard Drawdown</p>
            <p class="text-xl font-mono font-bold text-rose-400">6.0%</p>
          </div>
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Max Leverage</p>
            <p class="text-xl font-mono font-bold text-white/90">5x</p>
          </div>
          <div class="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p class="text-xs text-white/40 mb-1 font-sans">Kill Switch</p>
            <p class="text-xl font-mono font-bold text-emerald-400">WIRED</p>
          </div>
        </div>
        ...
        <div class="mt-5">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-white/40 font-sans">Current Drawdown</span>
            <span class="text-xs font-mono {dailyDrawdown > 3 ? 'text-rose-400' : 'text-emerald-400'}">{dailyDrawdown.toFixed(2)}%</span>
          </div>
          ...
          <div class="flex justify-between mt-1">
            <span class="text-[10px] text-white/30 font-mono">0%</span>
            <span class="text-[10px] text-amber-400/60 font-mono">3%</span>
            <span class="text-[10px] text-rose-400/60 font-mono">6%</span>
          </div>
        </div>
      </GlassCard>
        ...
        <div class="space-y-4">
          {#each progressMetrics as metric}
            <ProgressBar 
              ...
              showValue={true}
            >
              <span slot="label" class="text-xs text-white/40 font-sans">{metric.label}</span>
            </ProgressBar>
          {/each}
        </div>
      </GlassCard>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-white/[0.06]">
                <th class="text-left text-xs text-white/40 font-sans uppercase tracking-wider pb-3">Department</th>
                <th class="text-left text-xs text-white/40 font-sans uppercase tracking-wider pb-3">Direction</th>
                <th class="text-left text-xs text-white/40 font-sans uppercase tracking-wider pb-3">Confidence</th>
                <th class="text-left text-xs text-white/40 font-sans uppercase tracking-wider pb-3">Timeframe</th>
              </tr>
            </thead>
            <tbody>
              {#each departments as dept}
                <tr class="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  <td class="py-3 text-sm text-white/80 font-sans">{dept.department}</td>
                  <td class="py-3">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-mono font-semibold {dept.direction === 'long' ? 'bg-emerald-500/15 text-emerald-400' : dept.direction === 'short' ? 'bg-rose-500/15 text-rose-400' : 'bg-white/5 text-white/40'}">
                      {dept.direction.toUpperCase()}
                    </span>
                  </td>
                  <td class="py-3 text-sm font-mono text-white/70">{(dept.confidence * 100).toFixed(0)}%</td>
                  <td class="py-3 text-sm font-mono text-white/50">{dept.timeframe}</td>
                </tr>
              {:else}
                <tr>
                  <td colspan="4" class="py-8 text-center text-sm text-white/30 font-sans">No active signals</td>
                </tr>
              {/each}
            </tbody>
          </table>
        ...
        <div class="space-y-3">
          {#each Object.entries(marketPrices) as [symbol, price]}
            <div class="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
              <span class="text-sm font-sans text-white/70">{symbol.replace('USDT', '')}</span>
              <span class="text-sm font-mono font-semibold text-white/90">${price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          {:else}
            <div class="py-4 text-center text-sm text-white/30 font-sans">Loading prices...</div>
          {/each}
        </div>
      </GlassCard>
      ...
      <GlassCard title="System Status" subtitle="Infrastructure health">
        <div class="space-y-3">
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-white/50 font-sans">Daemon</span>
            <StatusBadge status={state ? 'online' : 'offline'} size="sm" />
          </div>
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-white/50 font-sans">Binance WS</span>
            <StatusBadge status={state?.binance_ws_connected ? 'online' : 'offline'} size="sm" />
          </div>
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-white/50 font-sans">Execution</span>
            <StatusBadge status={executionEnabled ? 'online' : 'error'} size="sm" />
          </div>
          <div class="flex items-center justify-between py-2">
            <span class="text-sm text-white/50 font-sans">KV Sync</span>
        ...
        <div class="space-y-3 max-h-64 overflow-y-auto scrollbar-hide">
          {#each recentActivity as activity}
            <div class="flex items-start gap-3 py-2 border-b border-white/[0.04] last:border-0">
              ...
              <div>
                <p class="text-xs text-white/70 font-sans">{activity.type || 'Event'}</p>
                <p class="text-[10px] text-white/30 font-mono mt-0.5">{activity.timestamp ? new Date(activity.timestamp).toLocaleTimeString() : 'Just now'}</p>
              </div>
            </div>
          {:else}
            <div class="py-4 text-center text-sm text-white/30 font-sans">No recent activity</div>
          {/each}
        </div>
      </GlassCard>
      ...
      <GlassCard title="Quick Actions">
        <div class="grid grid-cols-2 gap-3">
          <a href="/trade" class="flex flex-col items-center gap-2 p-4 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-emerald-500/30 transition-all group">
            ...
            <span class="text-xs font-medium text-white/70">New Trade</span>
          </a>
          <a href="/positions" class="flex flex-col items-center gap-2 p-4 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-blue-500/30 transition-all group">
            ...
            <span class="text-xs font-medium text-white/70">Positions</span>
          </a>
        </div>
      </GlassCard>
    </div>
