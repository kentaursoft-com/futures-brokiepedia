<script lang="ts">
	...
	const mockTrades: Trade[] = [
		{ id: '1', strategy_id: 'ema-v1', exchange: 'Binance', symbol: 'BTC-PERP', side: 'long', entry_price: 42850, exit_price: 43200, size: 0.1, pnl: 35, fee: 2.15, regime: 'trending_up', paper: false, created_at: Date.now() - 86400000 * 2 },
		{ id: '2', strategy_id: 'rsi-v1', exchange: 'Bybit', symbol: 'ETH-PERP', side: 'short', entry_price: 2580, exit_price: 2550, size: 0.5, pnl: 15, fee: 1.28, regime: 'ranging', paper: false, created_at: Date.now() - 86400000 * 1.5 },
		{ id: '3', strategy_id: 'ema-v1', exchange: 'Binance', symbol: 'BTC-PERP', side: 'long', entry_price: 43100, exit_price: 43050, size: 0.15, pnl: -7.5, fee: 2.58, regime: 'trending_up', paper: false, created_at: Date.now() - 86400000 },
		{ id: '4', strategy_id: 'funding-v1', exchange: 'Bitget', symbol: 'SOL-PERP', side: 'long', entry_price: 98.5, exit_price: 101.2, size: 2, pnl: 54, fee: 1.99, regime: 'high_volatility', paper: true, created_at: Date.now() - 43200000 },
		{ id: '5', strategy_id: 'ema-v1', exchange: 'Binance', symbol: 'BTC-PERP', side: 'short', entry_price: 43300, exit_price: 43150, size: 0.1, pnl: 15, fee: 2.16, regime: 'trending_down', paper: false, created_at: Date.now() - 21600000 }
	];
	...
	const symbols = ['all', ...new Set(mockTrades.map(t => t.symbol))];
</script>
	...
	<div class="flex items-center justify-between mb-4">
		...
		<div class="flex items-center gap-4 text-xs">
			<div class="flex items-center gap-1">
				...
				<span class="font-semibold {stats.wins / stats.total >= 0.55 ? 'text-emerald-400' : 'text-amber-400'}">
		...
		<table class="w-full text-sm">
			<thead>
				<tr class="border-b text-muted-foreground text-xs">
					...
					<th class="pb-2 text-right font-medium">Fee</th>
					<th class="pb-2 text-center font-medium">Type</th>
					<th class="pb-2 text-left font-medium">Regime</th>
				</tr>
			</thead>
			<tbody>
				{#each filteredTrades as trade}
					<tr class="border-b border-border/50 hover:bg-secondary/30 transition-colors">
						...
						<td class="py-2 text-right text-muted-foreground">${trade.fee.toFixed(2)}</td>
						...
						<td class="py-2 text-xs text-muted-foreground">{trade.regime}</td>
					</tr>
				{/each}
			</tbody>
		</table>
