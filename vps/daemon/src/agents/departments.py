"""LangGraph department agents for Futures Brokiepedia."""
import json
import logging
...
class DepartmentAgents:
    """6 department analysis agents using DeepSeek."""
    ...
    async def _aggregate_signals(self, state: AnalysisState) -> AnalysisState:
        ...
        logger.info(f"Aggregated signal: {direction} (conf: {confidence:.2f})"
                   f" size_reduction: {size_reduction}")
